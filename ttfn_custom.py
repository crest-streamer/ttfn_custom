...
wakeup_message = config.WakeupMessage
...
class Bot(commands.Bot):
    ...
    async def event_message(self, msg):
        ...
        # @ユーザー名を削除
        atesaki = re.search(r'@\S+', message)
        if atesaki:
            atesaki = atesaki.group()
        ...
        in_text = message
        if atesaki != None:
            print(in_text.replace('=~=',atesaki),flush=True)
        else:
            print(in_text,flush=True)
        ...
        if config.Debug:
            print(f'lang_detect:{lang_detect}')
                ...
                try:
                    if lang_detect in deepl_lang_dict.keys() and lang_dest in deepl_lang_dict.keys():
                        ...
                        if not config.GAS_URL:
                            try:
                                translatedText = await translate_text_async(in_text, lang_detect, lang_dest)
        ...
        if atesaki != None:
            out_text = out_text.replace('@@@', atesaki)
